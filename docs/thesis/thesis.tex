...
%  Author: Jofre Geli de Fuenmayor             Submission: 15 June 2026
%
...
% Emit /ActualText so copy-paste, search indexing and screen readers receive
% correct Unicode (fixes XeTeX mapping the Times New Roman ';' glyph to the
% Greek question mark U+037E in the PDF text layer).
\XeTeXgenerateactualtext=1

% --- Language (XeLaTeX reads UTF-8 natively; fontenc handled by fontspec) ---
\usepackage[main=english,catalan,spanish]{babel}

% --- Page geometry (matches tfg_dtic_upf log: ~2.5cm margins) ---------------
\usepackage[a4paper,top=2.5cm,bottom=2.5cm,left=2.5cm,right=2.5cm]{geometry}

% --- Fonts (XeLaTeX + Times New Roman proper from C:\Windows\Fonts) ---------
\usepackage{fontspec}
...
% --- Chapter heading format: inline number, no "Chapter N" stacked preamble --
\usepackage{titlesec}
...
% --- Official UPF institutional cover (included as page 1) -------------------
\usepackage{pdfpages}

% --- Project repository permalinks -------------------------------------------
% Single point of truth for the public Git repository and the commit the
% thesis links against. To re-pin every footnote permalink to a newer release
% commit, change \repocommit only. \repofile{path} renders a typeset path that
% links to the file at that commit; \repolink{path}{text} links arbitrary text.
\newcommand{\repobase}{https://github.com/jofregeli/voice-to-guitar-brave}
\newcommand{\repocommit}{36ddd4c7767be9e0b364fb0b6af7480256bd34bb}
\newcommand{\repofile}[1]{\href{\repobase/blob/\repocommit/\detokenize{#1}}{\texttt{\detokenize{#1}}}}
\newcommand{\repolink}[2]{\href{\repobase/blob/\repocommit/\detokenize{#1}}{#2}}
...
\newcommand{\thesisFaculty}{Escola d'Enginyeria}
...
% Tighter vertical spacing around floats and captions to keep the main body
% within the page budget without removing director-requested figures/tables.
\setlength{\textfloatsep}{8pt plus 2pt minus 2pt}
\setlength{\intextsep}{8pt plus 2pt minus 2pt}
\setlength{\abovecaptionskip}{4pt}
\setlength{\belowcaptionskip}{2pt}

% --- Page style: page numbers at bottom-centre, no running headers ----------
% Overrides book-class default 'headings' style; the template requires
% bottom page numbers and no running chapter/section headers.
\pagestyle{plain}

% ============================================================================
\begin{document}

% --- Official UPF institutional cover: physical page 1, unnumbered ----------
\includepdf[pages=1]{CoverTFG2025.pdf}

\frontmatter
\pagenumbering{Roman}  % uppercase Roman numerals for front matter (template requirement)
\pagestyle{plain}

% --- Title page (UPF DTIC template layout) ----------------------------------
% Layout per the official UPF DTIC cover example: title at top, "Treball Fi
% de Grau de" + author in the middle, director/degree/year below, UPF logo at
% the bottom. No institution-text block above the title and no subtitle/date.
\begin{titlepage}
\thispagestyle{empty}
\centering
\vspace*{4cm}
{\huge \thesisTitle\par}
\vspace{4cm}
{\textsc{\large Treball Fi de Grau de}}\\[0.7cm]
{\Large \thesisAuthor}\\[3cm]
Directors: \thesisAdvisor\\[0.4cm]
\thesisDegree\\[0.4cm]
Curs 2025--2026
\vfill
\includegraphics[width=0.55\textwidth]{logo_upf.png}\\[0.5cm]
\end{titlepage}

% --- Acknowledgements --------------------------------------------------------
\chapter*{Acknowledgements}
\addcontentsline{toc}{chapter}{Acknowledgements}
The author would like to thank the project directors, \thesisAdvisor, for guidance throughout the iterative development and for the methodological flagging of the dataset heterogeneity concern analysed in \S\ref{sec:noise_floor}. Acknowledgement is also due to the ACIDS-IRCAM research group for releasing the RAVE and BRAVE architectures, training code, and pretrained checkpoints used as the project's external baseline, and to the maintainers of the GuitarSet, Guitar-TECHS, IDMT-SMT-Guitar and Groove MIDI Dataset corpora.

% --- Abstracts ---------------------------------------------------------------
\chapter*{Abstract}
\addcontentsline{toc}{chapter}{Abstract}
Neural audio synthesis models --- from autoregressive networks such as WaveNet to the real-time variational autoencoder RAVE and its streaming-causal successor BRAVE --- have made expressive timbre transfer feasible on consumer hardware, raising the prospect of driving a musical instrument directly with the human voice. This thesis investigates whether the streaming-causal BRAVE architecture can deliver voice-driven instrument resynthesis at sub-10\,ms latency when trained on heterogeneous (mixed-source, differing recording conditions), limited-scale public datasets of guitar and drum audio. Six guitar and two drum training iterations were evaluated under a custom diagnostic framework. The principal outcome is a guitar autoencoder that partially reconstructs in-distribution audio but does not generalise to voice, which lies outside the training distribution. Reusing the same configuration for a drums model did not transfer cleanly. On the same diagnostic, the official ACIDS-IRCAM percussion baseline was more responsive than the project's drums model by more than two orders of magnitude, locating the limitation in training recipe and data scale rather than in the BRAVE architecture. The principal contributions are a five-mode failure taxonomy for RAVE-family models on small corpora, a cross-checkpoint calibration exposing a limitation of the STFT-correlation diagnostic, and an external-baseline disentanglement of architectural from training-recipe failure.

\chapter*{Resum}
\addcontentsline{toc}{chapter}{Resum}
\selectlanguage{catalan}%
Els models de síntesi neuronal d'àudio --- des de xarxes autoregressives com WaveNet fins a l'autoencoder variacional en temps real RAVE i el seu successor causal-streaming BRAVE --- han fet viable la transferència expressiva de timbre en maquinari de consum, obrint la possibilitat de controlar un instrument musical directament amb la veu humana. Aquesta tesi investiga si l'arquitectura BRAVE de tipus causal-streaming pot oferir resíntesi d'instruments controlada per veu amb una latència inferior a 10\,ms entrenada sobre conjunts de dades públics, heterogenis (de fonts diverses i amb condicions de gravació diferents) i de mida limitada. S'han avaluat sis iteracions de guitarra i dues de bateria sota un marc diagnòstic propi. El resultat principal és un autoencoder de guitarra que reconstrueix parcialment àudio en distribució, però no generalitza a la veu, que queda fora de la distribució d'entrenament. La reutilització de la mateixa configuració per a un model de bateria no es va transferir netament. Sobre el mateix diagnòstic, la línia base oficial ACIDS-IRCAM de percussió va ser més responsiva que el model de bateria del projecte per més de dos ordres de magnitud, situant la limitació en la recepta d'entrenament i no en l'arquitectura BRAVE. Les contribucions principals són una taxonomia de cinc modes de fallada per a models de la família RAVE en corpus petits, una calibració que exposa una limitació de la mètrica de correlació espectral, i una desambiguació mitjançant una línia base externa.
\selectlanguage{english}%

\chapter*{Resumen}
\addcontentsline{toc}{chapter}{Resumen}
\selectlanguage{spanish}%
Los modelos de síntesis neuronal de audio --- desde redes autorregresivas como WaveNet hasta el autoencoder variacional en tiempo real RAVE y su sucesor causal-streaming BRAVE --- han hecho viable la transferencia expresiva de timbre en hardware de consumo, abriendo la posibilidad de controlar un instrumento musical directamente con la voz humana. Esta tesis investiga si la arquitectura BRAVE de tipo causal-streaming puede ofrecer resíntesis de instrumentos controlada por voz con una latencia inferior a 10\,ms cuando se entrena sobre conjuntos de datos públicos, heterogéneos (de fuentes diversas y con condiciones de grabación diferentes) y de tamaño limitado. Se evaluaron seis iteraciones de guitarra y dos de batería bajo un marco diagnóstico propio. El resultado principal es un autoencoder de guitarra que reconstruye parcialmente audio en distribución, pero no generaliza a la voz, que queda fuera de la distribución de entrenamiento. La reutilización de la misma configuración para un modelo de batería no se transfirió limpiamente. Sobre el mismo diagnóstico, la línea base oficial ACIDS-IRCAM de percusión fue más responsiva que el modelo de batería del proyecto por más de dos órdenes de magnitud, situando la limitación en la receta de entrenamiento y no en la arquitectura BRAVE. Las contribuciones principales son una taxonomía de cinco modos de fallo para modelos de la familia RAVE en corpus pequeños, una calibración que expone una limitación de la métrica de correlación espectral, y una desambiguación mediante una línea base externa.
\selectlanguage{english}%

% --- Generative AI disclosure ------------------------------------------------
\chapter*{Use of Generative AI Disclosure}
\addcontentsline{toc}{chapter}{Use of Generative AI Disclosure}
The Author, \textbf{Jofre Geli de Fuenmayor}, discloses that the development and writing of this thesis involved the use of generative artificial intelligence tools, specifically \textbf{Claude (Anthropic)} and \textbf{ChatGPT (OpenAI)}.

Claude (Anthropic) was used to assist with the organization and structuring of the GitHub repository, as well as the creation of development scripts and other software engineering support tasks. ChatGPT (OpenAI) was used to support the improvement of the thesis writing, including language revision, text editing, and enhancement of the overall structure and clarity of the document.

The Author warrants, however, that the contents of this thesis constitute a genuine human creation. All outputs generated by Claude (Anthropic) and ChatGPT (OpenAI) were critically reviewed, validated, and, where necessary, modified by Jofre Geli de Fuenmayor. The Author assumes full responsibility for the final content, methodology, results, and conclusions presented in this work.

\textbf{Information source disclosure:} The Author further declares that all cited sources have been personally consulted and evaluated by Jofre Geli de Fuenmayor and have been sufficiently contrasted to ensure their reliability and appropriateness for academic use.
...
\begin{tikzpicture}[
    ...
    io/.style={rectangle, draw, rounded corners=2pt, minimum height=1.2cm, minimum width=2.4cm, align=center},
    model/.style={rectangle, draw, rounded corners=3pt, minimum height=1.7cm, minimum width=3.6cm, align=center, fill=blue!8},
    arrow/.style={-{Latex[length=2.5mm]}, very thick}
]
\node[io, fill=orange!12] (voice) {Human voice\\[2pt]\footnotesize vocal excitation};
\node[model, right=of voice] (brave) {BRAVE\\ causal autoencoder\\[2pt]\footnotesize trained on instrument\\\footnotesize audio only};
\node[io, fill=green!12, right=of brave] (inst) {Instrument timbre\\[2pt]\footnotesize guitar / drums};
\draw[arrow] (voice) -- (brave);
\draw[arrow] (brave) -- (inst);
\node[font=\footnotesize\itshape, below=0.55cm of brave, align=center, text width=7cm]
  {Target end-to-end latency $<10$\,ms for live use.\\
   Voice is \emph{out-of-distribution}: the model never sees it during training.};
\end{tikzpicture}}
\caption{Conceptual overview of the voice-to-instrument resynthesis task addressed in this thesis. The human voice drives a streaming-causal autoencoder (BRAVE) that is trained only on instrument audio; the system must render the vocal input in the target instrument's timbre, in real time, even though voice is an out-of-distribution input for the trained model. The technical signal path is detailed later in Figure~\ref{fig:block_diagram}.}
\label{fig:concept}
\end{figure}
...
RQ1 and RQ3 are answered constructively across the iteration sequence reported in Chapter~\ref{chap:results}: a guitar autoencoder that partially reconstructs in-distribution audio is obtained and five distinct training-failure modes are isolated. RQ2 is answered negatively, in a qualified form: instrument-only training did not yield perceptually convincing voice-to-instrument resynthesis on the small open corpora used here, and the IRCAM percussion baseline (\S\ref{sec:ircam_baseline}) suggests this is a training-recipe and data-scale limitation rather than an architectural impossibility.

\section{Objectives and Scope}\label{sec:objectives}

To address the research questions, this project defines a general objective to design and evaluate a streaming neural resynthesis pipeline for expressive voice-driven instrumental synthesis. The investigation is organised according to the following specific objectives:
...
\section{Principal Contributions}\label{sec:contributions}

The following are the principal contributions of this work. Each is grounded in the experimental evidence reported in Chapter~\ref{chap:results} and the methodological framework of Chapter~\ref{chap:methodology}:

\begin{itemize}
\item \textbf{Principal contribution 1 --- Failure-mode taxonomy.} A systematic, evidence-anchored account of five distinct training-failure modes observed across an eight-iteration training sequence: posterior collapse (v1), latent underutilisation (v2), discriminator dominance / generator mode collapse (v3, v4, drums\_v2), discriminator collapse (v5), and out-of-distribution decoder behaviour (v6, drums\_v1). To the author's knowledge, this is a more granular failure-mode breakdown within a single RAVE/BRAVE training programme than is commonly reported, although no systematic literature search was conducted to establish this as a formal novelty claim.
\item \textbf{Principal contribution 2 --- Calibration of the STFT-magnitude-correlation diagnostic.} A cross-checkpoint calibration study (\S\ref{sec:stft_calibration}, Table~\ref{tab:stft_calibration}) showing that the STFT-magnitude-correlation metric initially proposed as a working acceptance heuristic does not robustly discriminate working autoencoders from collapsed ones, because collapsed checkpoints produce trivially correlated near-silent outputs. This is a methodological finding that constrains how the metric should be used in future replication work.
\item \textbf{Principal contribution 3 --- External-baseline disentanglement.} A side-by-side diagnostic comparison between drums\_v1 and the official ACIDS-IRCAM percussion checkpoint (\S\ref{sec:ircam_baseline}) measured under identical conditions in Pure Data, isolating the observed OOD limitation as a training-recipe and dataset-scale issue rather than an architectural impossibility.
\item \textbf{Principal contribution 4 --- Partially-reconstructing in-distribution guitar autoencoder.} A streaming-causal RAVE v2 guitar autoencoder (\texttt{guitar\_v6\_best.ts}) recovered from a sequence of failed iterations, which partially reconstructs in-distribution guitar audio and serves as the reference point against which cross-modal vocal input is evaluated.
\item \textbf{Principal contribution 5 --- Dataset-heterogeneity quantification.} A noise-floor analysis (\S\ref{sec:noise_floor}, Table~\ref{tab:noise_floor}) measuring a 35.9\,dB spread across six recording paths within the consolidated guitar corpus, with documented implications for what features the encoder is likely to internalise as salient.
\item \textbf{Principal contribution 6 --- Open, reproducible Windows pipeline.} A version-controlled training, evaluation, and deployment pipeline (preprocessing, four \texttt{acids-rave} compatibility patches, training launchers, diagnostic and figure-generation scripts, Pure Data patches) released alongside this thesis so that the eight reported iterations can be re-run on commodity Windows hardware.
\end{itemize}

\section{Document Structure}\label{sec:doc_structure}

The thesis is organised into six chapters. Chapter~\ref{chap:sota} reviews the state of the art in neural synthesis and timbre transfer. Chapter~\ref{chap:methodology} details the methodology, focusing on system architecture and training protocols. Chapter~\ref{chap:implementation} describes the implementation of the Pure Data environment and the software pipeline. Chapter~\ref{chap:results} presents the evaluation results, including the failure taxonomy, the external-baseline validation, and the dataset heterogeneity analysis. Chapter~\ref{chap:conclusions} summarises the findings and proposes future work across calibrated latency measurement, formal perceptual evaluation, RAVE v3 architectures, mixed-domain training data, hybrid pipelines, and replication on larger corpora. The Pure Data demonstration setup is documented in Appendix-\ref{app:demo_setup}; reference audio samples are listed in Appendix-\ref{app:audio_samples}; configuration files and the four compatibility patches in Appendix-\ref{app:configs}; and the project planning artefacts (Project Charter, Work Breakdown Structure, phase schedule) in Appendix-\ref{app:planning}.
...
\textbf{Adversarial stabilisation techniques.} Stability of the adversarial phase depends on additional techniques from the generative-modelling literature: spectral normalisation of the discriminator \parencite{miyato2018spectralnorm}, multi-period discriminators inherited from HiFi-GAN \parencite{kong2020hifigan}, and multi-scale spectral losses computed over different STFT window sizes \parencite{engel2020ddsp}. Together, these strategies have established RAVE as one of the most widely adopted neural timbre-transfer architectures for interactive applications. The overall encoder--decoder structure and its two-phase training are shown in Figure~\ref{fig:rave_arch}.
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw, rounded corners=2pt, minimum height=1.0cm, minimum width=1.7cm, align=center},
    enc/.style={rectangle, draw, rounded corners=2pt, minimum height=1.0cm, minimum width=1.7cm, align=center, fill=blue!10},
    dec/.style={rectangle, draw, rounded corners=2pt, minimum height=1.0cm, minimum width=1.7cm, align=center, fill=green!10},
    lat/.style={rectangle, draw, rounded corners=2pt, minimum height=1.0cm, minimum width=1.6cm, align=center, fill=blue!20},
    disc/.style={rectangle, draw, rounded corners=2pt, minimum height=1.0cm, minimum width=2.4cm, align=center, fill=red!10},
    arrow/.style={-{Latex[length=2mm]}, thick}
]
\node[box] (in) {Input\\audio $x$};
\node[box, right=of in] (pqmf) {PQMF\\analysis\\\footnotesize 16 bands};
\node[enc, right=of pqmf] (enc) {Causal\\encoder};
\node[lat, right=of enc] (z) {Latent\\$z\in\mathbb{R}^{16}$};
\node[dec, right=of z] (dec) {Causal\\decoder};
\node[box, right=of dec] (synth) {PQMF\\synthesis};
\node[box, right=of synth] (out) {Output\\audio $\hat{x}$};

\draw[arrow] (in) -- (pqmf);
\draw[arrow] (pqmf) -- (enc);
\draw[arrow] (enc) -- (z);
\draw[arrow] (z) -- (dec);
\draw[arrow] (dec) -- (synth);
\draw[arrow] (synth) -- (out);

\node[draw, dashed, rounded corners=3pt, fit=(enc)(z)(dec), inner sep=0.3cm,
      label={[font=\footnotesize\itshape]above:Phase~1: multi-scale STFT reconstruction loss $+$ KL regularisation}] {};

\node[disc, below=1.6cm of dec] (disc) {Multi-scale\\discriminator};
\draw[arrow] (synth.south) |- (disc.east);
\draw[arrow, dashed] (in.south) |- (disc.west);
\node[font=\footnotesize\itshape, below=0.15cm of disc, align=center] {real vs.\ generated\\(Phase~2, encoder frozen)};
\end{tikzpicture}}
\caption{Simplified RAVE/BRAVE architecture and two-phase training. Input audio is split into 16 frequency bands by a PQMF analysis bank, encoded by a causal convolutional encoder into a low-dimensional Gaussian latent $z$, and reconstructed by a causal decoder followed by PQMF synthesis. In Phase~1 the encoder--decoder pair is trained to minimise a multi-scale STFT reconstruction loss with KL regularisation of the latent; in Phase~2 the encoder is frozen and the decoder is fine-tuned adversarially against a multi-scale discriminator. BRAVE differs from standard RAVE chiefly in using causal convolutions and a reduced compression ratio to enable sub-10\,ms streaming inference.}
\label{fig:rave_arch}
\end{figure}
...
RAVE (2021) & Yes & No & Tens of ms, config-dependent & Implicit & Learned VAE latent & Non-causal context limits perceptual immediacy \\
...
Classically, timbre is the perceptual characteristic that differentiates two sounds having the same pitch and loudness \parencite{ansi1994timbre}. It is usually treated as a multidimensional notion encompassing spectral envelope, attack and decay characteristics, fine-grained temporal modulations, and the resonant properties of the source. In machine learning, timbre transfer is the conversion of the timbral features of a source signal into those of a target instrument while preserving musical elements such as pitch, rhythm, and dynamics.
...
\paragraph{Groove MIDI Dataset.} \parencite{gillick2019groove} provides a percussive analogue to the guitar collections: approximately 13 hours of acoustic drum-kit performances by professional drummers with synchronised MIDI and audio across a wide range of styles and tempos; the portion retained for this project after preprocessing is approximately 10.86 hours (\S\ref{sec:datasets_preprocessing}). Beyond comfortably exceeding the lower bound for RAVE training, it also supports the secondary case study of this thesis.

\paragraph{Dataset selection rationale and noise-floor heterogeneity.} After selection for compatible files (mono, 16-bit, 44.1\,kHz), the consolidated guitar corpus (GuitarSet, the Guitar-TECHS DI channel and the continuous-performance subsets of IDMT-SMT-Guitar) is approximately 9 hours of raw audio, expanding to approximately 16 hours in the training database after RAVE's overlap segmentation (\S\ref{sec:datasets_preprocessing}). This duration lies within the recommended training window of \textcite{caillon2021rave} but introduces a well-known risk: mixing recording paths leads to inconsistency in background noise floors. The quantitative analysis presented in \S\ref{sec:noise_floor} reveals a 35.9\,dB range between the cleanest direct-input electric recordings and the loudest microphone-captured acoustic recordings. This range exceeds the typical dynamic variation within a single performance and may be encoded as a prominent latent feature, biasing the learned manifold toward recording-method discrimination rather than purely timbral content.
...
The principal design constraint is an end-to-end latency below 10\,ms, which lies within the approximate range identified by \textcite{wessel2002intimate} as the threshold for intimate musical control. The system uses Pure Data (Pd)\footnote{Pure Data, a real-time visual programming environment for audio: \url{https://puredata.info}.} combined with the \texttt{nn\textasciitilde} external\footnote{\texttt{nn\textasciitilde}, the ACIDS-IRCAM neural-inference external for Pure Data and Max: \url{https://github.com/acids-ircam/nn_tilde}.} \parencite{caillon2021rave}.
...
\paragraph{Phase 1: representation learning.} In the first phase, the model operates as a standard Variational Autoencoder \parencite{kingma2013vae}. The encoder and decoder are trained jointly to minimise a multi-scale Short-Time Fourier Transform (STFT) reconstruction loss comparing magnitude spectrograms of input and output at multiple temporal resolutions \parencite{engel2020ddsp,caillon2021rave}. The spectral loss is motivated by perceptual considerations: subtle phase variations of steady-state harmonic content are largely inaudible to human listeners, so the model is not penalised for them. This simplification has a known limit, however: phase coherence does matter for the faithful reconstruction of sharp transients and attack onsets --- precisely the plucked-string and percussive events most relevant to this work --- so a purely magnitude-based objective can leave audible smearing of transients that a phase-aware loss would better preserve. The same magnitude-only assumption carries over to the SMC reconstruction metric defined in \S\ref{sec:diagnostic_method}, which is by construction phase-invariant. The latent space is regularised through a Kullback--Leibler (KL) divergence term that encourages the posterior distribution to match a standard Gaussian prior \parencite{kingma2013vae}. The relative weight of this KL term is set by a coefficient $\beta$; the strength of the resulting (weighted) regularisation --- referred to throughout this thesis as the \emph{KL pressure} --- governs how strongly the encoder is pushed toward the prior at the expense of reconstruction fidelity. Applying a high KL pressure before the model has learned a useful representation is the direct cause of the posterior-collapse failure analysed below, which makes the scheduling of $\beta$ a central design decision.
...
\textit{Adopted solution.} All subsequent runs (v2--v6) replace the constant beta with a log-space KL warmup that begins at effectively zero pressure and ramps gradually toward its target over Phase~1. Iterations v2--v4 initialise beta at $1\times10^{-4}$ and ramp to 0.1 over the first one million steps; v5 lowers the target to the RAVE\,v2 default of 0.05 over 1.5\,M steps, and v6 inherits the official \texttt{v2.gin} schedule ($1\times10^{-6}$ initial, 0.05 target, a $20{,}000$-step warmup). The exact per-iteration values are those of the \texttt{.gin} configuration files listed in Table~\ref{tab:configs}.
...
\paragraph{Reproducibility.} All configuration files, preprocessing scripts and evaluation utilities used in the experiments were version-controlled throughout development and are released alongside the thesis as a public Git repository,\footnote{Project repository: \url{https://github.com/jofregeli/voice-to-guitar-brave}. All script permalinks in this thesis are pinned to a fixed commit of this repository.} together with the diagnostic Python scripts and the Pure Data deployment patches.
...
\begin{table}[H]
\centering
\caption{Composition of the guitar training corpus after format filtering (mono, 16-bit PCM, 44.1\,kHz). The IDMT-SMT-Guitar dataset~2 was deliberately set aside to hold every training source at a single native bit depth (16-bit PCM) and so avoid introducing a re-quantisation, and possible dithering, step into the corpus during baseline calibration. After preprocessing, the resulting LMDB reports \texttt{n\_seconds = 57665} ($\sim$16\,h); the higher figure reflects overlapping segmentation rather than additional source audio.}
...
\paragraph{Preprocessing pipeline.} A five-stage Python pipeline was implemented using \texttt{librosa}\footnote{\texttt{librosa}, a Python library for audio analysis: \url{https://librosa.org}.} and \texttt{soundfile}\footnote{\texttt{python-soundfile}, audio file I/O built on libsndfile: \url{https://python-soundfile.readthedocs.io}.}:
\begin{enumerate}
...
\item \textbf{Artefact filtering.} Metadata artefacts (notably \texttt{\_\_MACOSX/} \texttt{.wav} files inside Mac-archived datasets) and 24-bit files are removed. This validation stage was incorporated after some 24-bit files were observed to cause instability in the standard \texttt{rave preprocess} pipeline; standardising on natively 16-bit source material avoided this and kept the corpus to a single bit depth without a re-quantisation step.
...
\paragraph{What the evaluation can and cannot establish.} The framework rests on three classes of measurement --- in-training scalar metrics, synthetic-input responsiveness probes, and reconstruction comparison on in-distribution audio --- which together establish whether the optimisation is healthy, whether the encoder differentiates inputs, and whether the decoder produces audible non-degenerate output. They do not, by themselves, establish that the output is perceptually convincing, for two reasons that bound the perceptual claims made here: no formal listening test (MUSHRA, MOS, ABX) was conducted, so judgments of ``working'' or ``recognisable'' reconstruction are single-evaluator; and the SMC acceptance threshold of 0.3 is an empirical heuristic which \S\ref{sec:stft_calibration} shows does not robustly separate working from collapsed checkpoints. These limits constrain how Chapter~\ref{chap:results} should be read and are revisited in \S\ref{sec:method_limitations}; quantifying the SMC limitation is itself Principal Contribution~2 (\S\ref{sec:contributions}).
...
These five scalars are illustrated for the canonical \texttt{guitar\_v6} run in Figure~\ref{fig:tensorboard} (Appendix-\ref{app:configs}), which shows the healthy pattern against which the collapsed iterations of Chapter~\ref{chap:results} are diagnosed: a low, stable KL term, a multi-band spectral distance that decreases through Phase~1 and recovers after the Phase~2 transient, and discriminator logits that remain close rather than diverging.
...
\paragraph{Reconstruction validation.} A final diagnostic feeds in-distribution guitar audio drawn from GuitarSet through the trained autoencoder. Two measures are computed: the input/output amplitude ratio (target close to 1.0) and the \emph{STFT Magnitude Correlation} (SMC) between input and output. The SMC is the Pearson correlation coefficient between the flattened magnitude spectrograms of the input $x$ and the reconstruction $\hat{x}$:
\begin{equation}
\mathrm{SMC}(x,\hat{x}) =
\frac{\displaystyle\sum_{k,t}\big(|X|_{k,t}-\overline{|X|}\big)\big(|\hat{X}|_{k,t}-\overline{|\hat{X}|}\big)}
{\sqrt{\displaystyle\sum_{k,t}\big(|X|_{k,t}-\overline{|X|}\big)^{2}}\;\sqrt{\displaystyle\sum_{k,t}\big(|\hat{X}|_{k,t}-\overline{|\hat{X}|}\big)^{2}}},
\label{eq:smc}
\end{equation}
where $|X|_{k,t}=|\mathrm{STFT}(x)|_{k,t}$ is the magnitude of frequency bin $k$ at frame $t$ (parameters \texttt{n\_fft}$=2048$, hop$=512$), $|\hat{X}|$ is the magnitude STFT of the reconstruction, and $\overline{\,\cdot\,}$ denotes the mean over all bins and frames. A time-aligned variant, $\mathrm{SMC}_{\text{aligned}}$, reports the maximum of Equation~\eqref{eq:smc} over small frame shifts ($\pm 8$ frames) to absorb the model's group delay. The SMC is phase-invariant by construction, consistent with the phase-agnostic spectral training loss. As no widely standardised threshold exists in the published literature for ``perceptually acceptable'' neural audio reconstruction expressed as SMC, this thesis adopts a value above 0.3 as an empirical heuristic, derived from informal listening comparisons against checkpoints with lower correlations. This is treated as a pragmatic working threshold rather than a normative claim, and its limitations are quantified by the cross-checkpoint calibration in \S\ref{sec:stft_calibration}.

\paragraph{Rationale for the spectral-correlation metric, and why it was later calibrated.} The SMC was chosen initially because it is phase-invariant (consistent with the phase-agnostic training loss), cheap to compute over many candidate checkpoints, and a single bounded scalar convenient for ranking --- attractive properties for a first-pass automated screen. The need for calibration arose empirically: applied across the \emph{full} iteration sequence rather than a few hand-picked checkpoints, it failed to separate working from collapsed models, because a collapsed decoder's near-silent output still shares the broad spectral envelope of natural audio and so correlates trivially with the input. This motivated the cross-checkpoint calibration of \S\ref{sec:stft_calibration}; the failure of an initially reasonable metric is itself one of the principal contributions of this work (\S\ref{sec:contributions}).

\paragraph{Validity limitations of the diagnostic framework.} Four caveats apply and are noted here so the results of Chapter~\ref{chap:results} are read in context; they are developed further in \S\ref{sec:method_limitations}. (i)~Synthetic sine-wave probing is a structural test of input differentiation, not of perceptual realism: it verifies only that the network's internal mappings are non-degenerate. (ii)~The SMC criterion measures spectral-envelope similarity alone --- ignoring phase, micro-temporal modulation, and other perceptual cues --- and the calibration of \S\ref{sec:stft_calibration} shows it does not robustly separate working from collapsed checkpoints, since a near-silent output still shares the broad spectral envelope of natural audio; listening evaluation therefore became the primary perceptual evaluator. (iii)~The $[0.05, 0.3]$ amplitude band is an engineering sanity check for clipping and silent-output failures, not a perceptual loudness model. (iv)~No formal perceptual test (MUSHRA, MOS) was conducted; listening evaluation was informal and single-author. Together, these protocols allow each iteration to be evaluated against consistent quantitative criteria while remaining transparent about what those criteria do and do not measure.
...
\textbf{Cross-modal generalisation assumption.} The system is trained only on instrument audio yet is required to generalise to a categorically different excitation (voice) at inference time; there is no theoretical guarantee that it will represent vocal input meaningfully, and the project proceeds on the working assumption that learned latent representations carry enough spectral-temporal generality for at least partial cross-modal mapping. The failure of this assumption on small open corpora is itself one of the documented findings (\S\ref{sec:drums}, \S\ref{sec:ircam_baseline}).

\textbf{Dataset scale and heterogeneity.} The guitar corpus ($\sim$9~h raw, $\sim$16~h after overlap segmentation) is small by contemporary standards, and it mixes capture paths: the noise-floor analysis of \S\ref{sec:noise_floor} measures a 35.9\,dB spread \emph{between} the median noise floors of different source conditions --- a corpus-level inconsistency across recording paths, not a within-signal signal-to-noise ratio. The work neither tests whether the observed failure modes persist at larger scale nor controls for this heterogeneity, so the latent representation is likely to encode recording method as a salient feature (not verified by a controlled latent-clustering experiment); larger, more homogeneous corpora are identified as future work (\S\ref{sec:future_work}).

\textbf{Evaluation and reproducibility limits.} Listening evaluation was informal and single-author (no MUSHRA/MOS panel); the end-to-end latency of Table~\ref{tab:latency_budget} is an analytic sum of component latencies, not a calibrated loopback measurement; and each configuration was trained once, so seed-to-seed variation --- to which adversarial training is known to be sensitive --- was not characterised. Each observed outcome should therefore be read as one realisation rather than a population statistic. These points are revisited in \S\ref{sec:limitations}.

The implementation details that operationalise this methodology are described in the following chapter.
...
This chapter describes how the methodology of Chapter~\ref{chap:methodology} was operationalised in software and deployed for real-time use. Implementation choices are linked explicitly to the project's research objectives: sub-10\,ms latency, encoder responsiveness, and reproducible training. The hardware and software environment, together with the four \texttt{acids-rave 2.3.1} compatibility patches required to run the toolchain on current SciPy and PyTorch Lightning releases, are documented in Appendix-\ref{app:configs}.
...
To manage interruptions during multi-day runs, each iteration is launched through a Windows \texttt{.bat} script.\footnote{Representative launcher for the final guitar iteration: \url{https://github.com/jofregeli/voice-to-guitar-brave/blob/36ddd4c7767be9e0b364fb0b6af7480256bd34bb/scripts/train_guitar_v6.bat}. The launchers for the other iterations sit alongside it under \texttt{scripts/}.} The script invokes PowerShell to find the most recent checkpoint matching the run name; if one is found, training resumes via the \texttt{--ckpt} flag, otherwise it starts fresh. This made the pipeline safely interruptible and was used multiple times after both planned and unplanned system restarts.
...
All training, preprocessing and export in this project were carried out with \texttt{acids-rave}, the official open-source RAVE implementation released by ACIDS-IRCAM \parencite{caillon2021rave},\footnote{\texttt{acids-rave} (the \texttt{rave} command-line tool used for \texttt{preprocess}, \texttt{train} and \texttt{export}): \url{https://github.com/acids-ircam/RAVE}. Version 2.3.1 was used here, with the four compatibility patches of Appendix-\ref{app:configs}.} which provides the \texttt{rave} command-line interface together with the \texttt{.gin} configuration system used throughout Chapters~\ref{chap:methodology} and~\ref{chap:implementation}. Model deployment followed the standard \texttt{acids-rave} export workflow: the command \texttt{rave export --run <RUN\_DIR> --name <NAME>} compiles the trained PyTorch graph into a TorchScript \texttt{.ts} file that bundles the learned weights with the cached convolutional state required for incremental inference.
...
\begin{center}
{\footnotesize\texttt{adc\textasciitilde\ 1 $\rightarrow$ nn\textasciitilde\ <model>.ts forward $\rightarrow$ *\textasciitilde\ <gain> $\rightarrow$ dac\textasciitilde}}
\end{center}
...
The investigation is divided into two main case studies: a six-iteration development arc for guitar resynthesis and an additional evaluation of drum-kit resynthesis. As described in Chapter~\ref{chap:methodology}, pipeline performance was measured against diagnostic criteria for encoder separation (pairwise latent difference greater than 0.5), decoder sensitivity (pairwise output difference greater than 0.01), and energy conservation (output level within $[0.05, 0.3]$). In-distribution reconstruction was checked by an STFT Magnitude Correlation (SMC, Equation~\eqref{eq:smc}) rule of greater than 0.3. All live experiments were conducted within a Pure Data environment using the \texttt{nn\textasciitilde} external with an estimated end-to-end delay of approximately 7\,ms.

\section{Guitar: Six-Iteration Arc}\label{sec:guitar_arc}

The guitar example focused on upgrading the training procedure step by step, starting from the baseline settings and progressing to the official RAVE v2 architecture. Each step was instrumented through the diagnostic Python scripts described in Chapter~\ref{chap:implementation}. Before the per-iteration narrative, Table~\ref{tab:diagnostics} gathers the principal quantitative diagnostic values for every model --- the six guitar iterations, the two drums iterations, and the IRCAM external baseline --- in one place, so that the figures cited individually in the discussion below can be read against each other at a glance.

\begin{table}[H]
\centering
\caption{Consolidated synthetic-input diagnostic measurements across the six guitar iterations, the two drums iterations, and the IRCAM external baseline. Values are drawn from the per-model analyses of \S\ref{sec:guitar_arc}--\S\ref{sec:ircam_baseline}; ``---'' marks a quantity not reported for that model. Encoder and forward (output) differences are pairwise mean absolute differences over the synthetic-input probe set (\S\ref{sec:diagnostic_method}); the health thresholds are encoder difference $>0.5$ and forward difference $>0.01$. Output RMS is the mean output energy on the GuitarSet reconstruction set (input RMS $\approx 0.065$). The final column names the failure mode of each iteration; the five-mode taxonomy these populate is set out in \S\ref{sec:failure_taxonomy}.}
\label{tab:diagnostics}
\footnotesize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.5cm}ccc>{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Model} & \textbf{Enc.\ diff.} & \textbf{Fwd.\ diff.} & \textbf{Out RMS} & \textbf{Training signature and diagnostic outcome} \\
\midrule
guitar\_v1 & 1.25 & 0.006 & 0.005 & KL falls $0.83\!\to\!0.25$ --- posterior collapse \\
guitar\_v2 & 0.79 & 0.003 & 0.003 & KL/dim $\approx 0.005$ nats --- latent underutilisation \\
guitar\_v3 & 26.4 & 0.042 & 0.004 & Forward diff.\ peaks, then degrades --- Phase-2 generator collapse \\
guitar\_v4 & --- & --- & 0.021 & GAN gap $=5.65$ (\texttt{pred\_real} $3.34$, \texttt{pred\_fake} $-2.31$) --- adversarial mode collapse \\
guitar\_v5 & --- & --- & 0.024 & GAN gap $=0.80$ (\texttt{pred\_real} $-0.90$) --- discriminator collapse \\
guitar\_v6 (best) & 1.27--6.89\textsuperscript{a} & --- & 0.024 & GAN-gap reversal; SMC $=0.16$ --- partial in-distribution reconstruction \\
drums\_v1 & --- & 0.0045 & --- & Below tonal threshold --- partial transient response; voice OOD \\
drums\_v2 & --- & --- & --- & GAN gap widens $9.8\!\to\!20.3$ --- terminated (discriminator dominance) \\
IRCAM percussion\textsuperscript{b} & --- & 1.75 & 0.001\textsuperscript{c} & External baseline --- responsive to OOD voice \\
\bottomrule
\end{tabularx}\\[3pt]
{\footnotesize \textsuperscript{a}\,Encoder differences for guitar\_v6 are measured on out-of-distribution \emph{voice} inputs. \quad\textsuperscript{b}\,Official ACIDS-IRCAM checkpoint used as the external baseline (\S\ref{sec:ircam_baseline}). \quad\textsuperscript{c}\,Output amplitude for silent input.}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{../../figures/figure_5_1_guitar_v1_posterior_collapse.png}
...
In the next run (\texttt{guitar\_v2}) the $\beta$ warmup was applied and posterior collapse was no longer a problem. The model, however, did suffer a partial collapse because the latent space was too large. With \texttt{LATENT\_SIZE} of 128, the model achieved a utilisation of only approximately 0.005 nats per dimension. The diagnostics revealed a small encoder difference (0.79) and a decoder that was not reacting at all (forward difference $=0.003$). The mean output amplitude on the synthetic tonal probe was approximately 0.012, far below the $[0.05, 0.3]$ health band and consistent with the near-silent GuitarSet reconstruction RMS of 0.003 reported in Table~\ref{tab:diagnostics}. This implied that the 128-dimensional latent vector was too large for the $\sim$5.4-hour corpus available at that stage, and a reduction to 16 dimensions was therefore necessary.
...
\subsection{Iteration v6: Partial In-Distribution Reconstruction}\label{sec:guitar_v6}

The final iteration (\texttt{guitar\_v6}) was based on the RAVE v2 official architecture and \texttt{causal.gin} overrides. In Phase~2 the GAN logit gap was observed to reverse for the first time in the series, suggesting that an adversarial equilibrium had been reached. Listening tests indicated perceptually recognisable reconstruction of in-distribution guitar audio at epoch 1935 (mid-Phase-2), and this checkpoint was therefore selected as canonical.\footnote{Representative input/output audio pairs for in-distribution guitar reconstruction (\texttt{\_input\_guitar.wav}, \texttt{\_recon\_guitar\_v6.wav}, and three further \texttt{v6\_final} pairs) are provided in the repository so that the reconstruction quality described here can be heard directly: \url{https://github.com/jofregeli/voice-to-guitar-brave/tree/36ddd4c7767be9e0b364fb0b6af7480256bd34bb/examples}. These complement the spectrogram of Figure~\ref{fig:spectrogram_in_dist}, since the magnitude-only diagnostics of this chapter cannot convey perceptual quality on their own.} The final epoch (2511) showed slightly degraded performance under the same listening criteria. Under the operational diagnostic of \S\ref{sec:diagnostic_method}, output amplitude rose from the near-silent levels of the collapsed checkpoints v1--v3 (output RMS $\approx 0.003$--$0.005$) to RMS $\approx 0.024$ on the canonical guitar\_v6 checkpoint and 0.041 on the Phase~1 reconstruction-only state, which together suggest a meaningful recovery of decoder responsiveness.

A quantitative reconstruction test on the canonical checkpoint, using eight GuitarSet solo-microphone samples of five seconds each (STFT parameters: \texttt{n\_fft}$=2048$, hop$=512$), yielded a mean SMC of 0.16 (0.17 with time-shift alignment). This value falls below the 0.3 heuristic proposed initially in \S\ref{sec:diagnostic_method}. To understand whether this gap reflects a limitation of the model or of the metric, the same evaluation was applied to every preceding checkpoint in the iteration sequence; the results are reported in \S\ref{sec:stft_calibration}. Anticipating the finding of that calibration: the SMC does not differentiate working from collapsed checkpoints in the expected direction, and the operational evaluation of guitar\_v6 as a partially-reconstructing in-distribution autoencoder therefore rests on the combined evidence of (i)~listening tests, (ii)~output amplitude recovery, and (iii)~Phase~2 GAN gap reversal.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../../figures/figure_5_3_spectrogram_in_dist.png}
\caption{STFT magnitude spectrograms (log scale) for an in-distribution GuitarSet sample: input (left) and the canonical guitar\_v6 reconstruction (right). The reconstruction reproduces the broad spectral envelope of the input but loses fine detail at higher frequencies.}
...
To assess whether the STFT magnitude correlation criterion of \S\ref{sec:diagnostic_method} robustly discriminates working autoencoders from collapsed ones, the same in-distribution reconstruction test was applied to every checkpoint in the iteration sequence.\footnote{Calibration script: \url{https://github.com/jofregeli/voice-to-guitar-brave/blob/36ddd4c7767be9e0b364fb0b6af7480256bd34bb/scripts/guitar_v6_stft_correlation.py}.} Eight GuitarSet solo-microphone samples of five seconds each were evaluated against each checkpoint with identical STFT parameters (\texttt{n\_fft}$=2048$, hop$=512$). Results are summarised in Table~\ref{tab:stft_calibration}.

\begin{table}[H]
\centering
\caption{Cross-checkpoint calibration of the STFT Magnitude Correlation (SMC, Equation~\eqref{eq:smc}) diagnostic. All values are means across $N=8$ GuitarSet samples. Output RMS reflects the energy of the model output relative to the input (mean input RMS $\approx 0.065$).}
...
\toprule
\textbf{Checkpoint} & \textbf{Diagnostic state} & {\textbf{Output RMS}} & {\textbf{SMC (raw)}} & {\textbf{SMC (aligned)}} \\
\midrule
...
The IRCAM model respected silence (0.001 output for silent input) and, in informal Pure Data listening tests with voice input, produced audibly modulated percussive output. This suggests that a well-trained BRAVE model with higher-quality or more voluminous data may overcome the manifold-mapping bottleneck. This single-baseline comparison is consistent with the interpretation that the performance-limiting factors of the present models are training recipe and data scale rather than fundamental architectural constraints of BRAVE, though it cannot establish this conclusively.
...
Chapter~\ref{chap:methodology}, \S\ref{sec:datasets_preprocessing} introduced the heterogeneity concern that arises from combining recordings captured through different paths into a single training corpus. To characterise that heterogeneity quantitatively, the noise-floor methodology defined in \S\ref{sec:datasets_preprocessing} was applied to every source recording of the consolidated guitar corpus (919 files across six recording paths) by the script \repofile{scripts/analyze_noise_floor.py}. For each file, the Root Mean Square energy was computed in non-overlapping 100\,ms windows; the 5th percentile of these RMS values was taken as the noise-floor estimate and converted to dBFS, following Equation~\eqref{eq:noisefloor}. This estimator assumes that at least the quietest 5\,\% of a recording corresponds to inter-note gaps, decay tails, or pre/post-roll silence rather than to sustained signal; the assumption is therefore content-dependent and is most reliable for the sparse, note-by-note and solo material that dominates the guitar corpus, and would over-estimate the floor for continuously dense or heavily rhythmic passages. It is used here as a comparative measure across sources captured under fixed conditions rather than as an absolute acoustic noise-floor figure. Per-source aggregates are reported in Table~\ref{tab:noise_floor} and visualised in Figure~\ref{fig:noise_floor}. The analysis was performed on the source recordings; the 52 Guitar-TECHS files measured here were reduced to 32 in the final training corpus of Table~\ref{tab:guitar_corpus} (899 files in total) after 20 stereo files were removed during the mono-conversion stage, so the two totals differ only by that filtering step.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../../figures/figure_5_4_noise_floor.png}
...
This thesis explored the feasibility of the streaming-causal BRAVE architecture for voice-driven instrument resynthesis with less than 10\,ms latency when trained on heterogeneous, limited-scale datasets, across six guitar iterations (v1--v6) and two drums iterations (\texttt{drums\_v1}, \texttt{drums\_v2}). The principal findings, organised by research question, are the following.

\begin{itemize}
\item \textbf{In-distribution feasibility (RQ1).} \texttt{guitar\_v6} --- built on the official \texttt{v2.gin} and \texttt{causal.gin} configurations with a reduced latent size of 16 and a longer Phase~1 of 1.5\,M steps --- was the first iteration to partially reconstruct in-distribution guitar audio (\S\ref{sec:guitar_v6}). The drums case was weaker: \texttt{drums\_v1} produced only a partial transient response and \texttt{drums\_v2} was stopped prematurely when Phase~2 discriminator dominance re-emerged despite an \texttt{update\_discriminator\_every=4} schedule.
\item \textbf{Cross-modal generalisation (RQ2).} Voice remained an out-of-distribution (OOD) input across all guitar checkpoints. The encoder distinguished vocal inputs (latent differences from 1.27 to 6.89), but the decoder output was near-identical across them --- a structural failure to associate voice latents with the trained guitar manifold rather than a failure of the encoder.
\item \textbf{Architecture versus training recipe (RQ2).} The OOD limitation appears attributable to the training recipe and data scale rather than to a design flaw of BRAVE. The IRCAM percussion baseline was far more reactive (forward difference 1.75 versus 0.0045 for \texttt{drums\_v1}, more than two orders of magnitude), while respecting silence (output $\approx 0.001$ for silent input).
\item \textbf{Failure-mode taxonomy (RQ3).} Iterating through the training cycle characterised five failure modes of RAVE-family models on small datasets: (i)~posterior collapse (v1), (ii)~latent underutilisation (v2), (iii)~discriminator dominance / generator mode collapse (v3, v4, \texttt{drums\_v2}), (iv)~discriminator collapse (v5), and (v)~OOD decoder behaviour (v6, \texttt{drums\_v1}).
\end{itemize}

\paragraph{The decisive role of the datasets.} A recurring thread runs through all of the findings above: the quantity and homogeneity of the training data, more than the specific architecture or any single hyperparameter, governed what each model could and could not do. Three independent lines of evidence point the same way. First, the early latent failures (v1--v2) were tied to a corpus too small to support a 128-dimensional latent, and were only relieved once the latent was matched to the data scale. Second, the noise-floor analysis (\S\ref{sec:noise_floor}) measured a 35.9\,dB spread across recording paths in the guitar corpus, a heterogeneity the encoder is well positioned to encode as a salient feature at the expense of timbral discrimination --- a dimension along which a single-source, curated corpus would not penalise the model. Third, and most directly, the IRCAM percussion baseline --- presumably trained on a larger and more curated corpus --- was responsive to OOD voice on the identical diagnostic where the project's drums model was not, despite sharing the same architecture family. Taken together, these observations reframe the central limitation of this work as a data problem rather than an architectural one: the BRAVE family appears capable of the target behaviour, but realising it depends on a training corpus of sufficient scale and, critically, recorded under sufficiently uniform conditions. This conclusion directly motivates the larger-corpus and homogeneity-controlled replication identified in \S\ref{sec:future_work}, and it is the reason dataset curation --- not further architectural tuning --- is argued here to be the highest-leverage next step.

\section{Contributions}\label{sec:contributions_summary}

The principal contributions of this work are enumerated in full in \S\ref{sec:contributions}; in the light of the results they can be summarised under three headings. First, a reproducible diagnostic methodology: a step-by-step procedure that compares responsive and collapsed checkpoints, with concrete latent- and output-difference thresholds, to verify a model's internal representations beyond informal listening (\S\ref{sec:diagnostic_method}). Second, a five-mode failure taxonomy that maps each TensorBoard signature (KL drift, real/fake logit gaps, output-amplitude degeneration) to its underlying optimisation cause, intended as a troubleshooting guide for RAVE-family training on small or heterogeneous corpora. Third, an external-baseline benchmarking that locates the OOD limitation in training data and recipe rather than in the BRAVE architecture, implying that the cross-modal gap can be narrowed with a better training corpus rather than a new model.
...
Beyond these specific points, three limitations of experimental rigour apply to the study as a whole and should be read alongside the results of Chapter~\ref{chap:results}. First, \textbf{no statistical significance testing} was performed: the diagnostic values reported (forward differences, KL trajectories, logit gaps, STFT correlations) are single measurements without confidence intervals or hypothesis tests, so apparent differences between iterations should be read as indicative rather than statistically established. Second, \textbf{no controlled listener study} was conducted: all perceptual judgments are single-evaluator and informal, as already noted, with no multi-participant panel, hidden reference, or blind protocol. Third, \textbf{each configuration was trained only once} (a single random seed); because adversarial training is known to be seed-sensitive, the outcomes of individual iterations should be understood as one realisation rather than as a characterisation of expected behaviour across seeds. These constraints follow from the exploratory, resource-bounded nature of the project and are revisited as priorities for future work in \S\ref{sec:future_work}.
...
Besides, a proper perceptual evaluation should be carried out to assess the in-distribution reconstruction quality of \texttt{guitar\_v6}. The design of such a test is itself non-trivial for this task. A full MUSHRA protocol requires a hidden reference and a low-quality anchor; for in-distribution guitar reconstruction the clean source recording is a natural reference, but a principled anchor is harder to define, since the conventional 3.5\,kHz low-pass anchor does not represent the failure modes (transient smearing, near-silent collapse) characteristic of these models --- a low-amplitude or earlier-collapsed checkpoint would arguably be a more meaningful anchor. Given this difficulty, and the fact that voice-driven output is out-of-distribution and has no ground-truth target at all, a lighter paired-comparison protocol (AB or ABX between checkpoints, or between model output and reference) would be a more tractable first step than a full MUSHRA panel, and would already yield a statistically grounded comparison of reconstruction quality across iterations and against other real-time synthesis methods.
...
The project began as an attempt to build a working voice-to-guitar prototype and was redirected mid-way into a documented case study and failure analysis, once it became clear that recognisable in-distribution reconstruction was achievable but expressive out-of-distribution control via voice was not, under the tested training conditions. The reusable diagnostic framework and the systematic failure taxonomy that resulted are argued here to be a more robust foundation for future real-time neural-synthesis research than an isolated successful demonstration would have been.
...
\item Synthetic drum samples used for the hybrid-pipeline preliminary exploration --- vocal-percussion onset classification routed to per-class drum samples, identified as future work in \S\ref{sec:future_work} --- (kick, snare, hi-hat one-shots), generated procedurally by \texttt{scripts/vpt\_make\_samples.py}.
\end{itemize}

\chapter{Environment, Configurations and Compatibility Patches}\label{app:configs}

\section{Hardware and software environment}\label{sec:env_compat}

The BRAVE resynthesis pipeline \parencite{caillon2021rave} was deployed on a single workstation. The complete software and hardware stack is summarised in Table~\ref{tab:env}. During training, the RTX 5080 supported an observed throughput of 9--12 optimisation steps per second, so each full 3,000,000-step run required 90--120 wall-clock hours, which made GPU capacity the primary constraint on iteration speed.
...
\section{Example training dashboard}

Figure~\ref{fig:tensorboard} reproduces the five primary TensorBoard scalars (\S\ref{sec:diagnostic_method}) for the canonical \texttt{guitar\_v6} run, as a reference for the diagnostic signatures discussed throughout Chapter~\ref{chap:results}.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../../figures/figure_tensorboard_scalars.png}
\caption{Example TensorBoard training scalars for the canonical \texttt{guitar\_v6} run: KL regularisation, multi-band spectral distance, the two discriminator logits (\texttt{pred\_real}, \texttt{pred\_fake}) and the total discriminator loss. The dashed vertical line marks the Phase~1$\rightarrow$Phase~2 transition ($\sim$1.5\,M steps); faint traces are the raw scalars and solid traces an exponential moving average. This is the dashboard view from which the diagnostic signatures of Chapter~\ref{chap:results} are read.}
\label{fig:tensorboard}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{../../figures/figure_C2_drums_v2_gan_widening.png}
\caption{drums\_v2 GAN logit gap over the duration of training (referenced from \S\ref{sec:drums}). Phase~1 (steps 0--1.5\,M) maintains a near-zero adversarial gap. From Phase~2 onset onward, the gap widens monotonically from approximately 9.8 to over 20, reproducing the discriminator-dominance pattern previously observed in iterations guitar\_v3 and guitar\_v4 despite the use of \texttt{update\_discriminator\_every=4}.}
\label{fig:drums_v2_gan}
\end{figure}

\section{Iteration configuration files}

The custom \texttt{.gin} configuration files used across the training iterations are listed in Table~\ref{tab:configs} of Chapter~\ref{chap:implementation}. The full text of the five custom configurations is reproduced in the project repository under \texttt{config/}; the unmodified \texttt{c128\_r10.gin} baseline and the official \texttt{v2.gin} and \texttt{causal.gin} files on which the later iterations build ship with \texttt{acids-rave} and are not duplicated there.
...
\medskip\noindent\textbf{Patch 1 --- \texttt{pqmf.py} line 10: \texttt{scipy.signal.kaiser} relocated.}\par\nobreak
...
\medskip\noindent\textbf{Patch 2 --- \texttt{pqmf.py} line 67: \texttt{kaiserord} argument type.}\par\nobreak
\begin{lstlisting}[language=Python]
# Before
...
\medskip\noindent\textbf{Patch 3 --- \texttt{pqmf.py} line 70: \texttt{firwin} \texttt{nyq} parameter.}\par\nobreak
\begin{lstlisting}[language=Python]
# Before
...
\medskip\noindent\textbf{Patch 4 --- \texttt{model.py} line 445: PyTorch Lightning 2.x API rename.}\par\nobreak
\begin{lstlisting}[language=Python]
# Before
...
The main elements of the Project Charter are summarised in this section; the project planning documents are also tracked in the repository under \texttt{docs/} (\texttt{workplan.md}, \texttt{gantt.md}, \texttt{timeline\_and\_decisions.md}).

\paragraph{Project identification.} Title: \emph{BRAVE-Based Real-Time Voice-to-Instrument Resynthesis}. Programme: \thesisDegree, \thesisInstitution, \thesisFaculty. Author: \thesisAuthor. Directors: \thesisAdvisor. Period: December 2025 -- June 2026 (active iteration cycle: 24 March -- 15 June 2026; see \emph{Project history and scope reset} below). ECTS: 20 (approximately 500 hours of student work).
...
\paragraph{Deliverables.} Project Charter, Work Breakdown Structure, phase schedule and Gantt chart (this appendix); trained autoencoder checkpoints (TorchScript); diagnostic Python scripts; Pure Data deployment patches; failure taxonomy (\S\ref{sec:failure_taxonomy}); final memoir (this document); defence presentation; public Git repository.
...
\item Guitar Case Study --- Iterative Training (Execution phase)
  ...
  \item \texttt{guitar\_v6} -- official \texttt{v2.gin} $+$ \texttt{causal} $\to$ partial in-distribution reconstruction
  \end{enumerate}
...
\item External Baseline and Validation Studies (Execution phase)
  ...
  \item Hybrid pipeline preliminary exploration (vocal-percussion onset classification)
  \end{enumerate}
\item Documentation (Final phase)
  ...
  \item Appendix-A (Demo Setup), Appendix-B (Audio Samples), Appendix-C (Configurations and Compatibility Patches), Appendix-D (Project Planning)
  \end{enumerate}
\item Submission and Defence (Final phase)
  ...
  \item Public repository release (public at submission; permalinks pinned to a fixed release commit)
...
\begin{table}[H]
\centering
\caption{Three-phase schedule of the active iteration cycle (24~March -- 15~June 2026), following the UPF TFG guide \parencite{hernandezleo2012tfg}. December 2025 -- 23~March 2026 was an earlier scoping and first (unsuccessful) environment-setup phase, described above. Phase boundaries correspond to the dated training runs in the project repository.}
\label{tab:phases}
\begin{tabular}{lccc}
\toprule
...
Final (writing + defence preparation) & 3 weeks & $\sim$30\,\% & 22 May -- 15 June \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:gantt} renders the active iteration cycle of Table~\ref{tab:phases} as a Gantt chart, mapping the principal tasks to the twelve weeks between 24~March and 15~June 2026 and to the three project phases. Shaded cells indicate the weeks of active work on each task; phase boundaries are approximate at the weekly resolution shown.

\begin{table}[H]
\centering
\caption{Gantt chart of the active iteration cycle (24~March -- 15~June 2026). Shaded cells mark weeks of active work on each task; the phase bands follow the schedule of Table~\ref{tab:phases}.}
\label{tab:gantt}
\footnotesize
\setlength{\tabcolsep}{1.5pt}
\renewcommand{\arraystretch}{1.25}
\newcommand{\gb}{{\color{blue!55}\rule{4.6mm}{2.6mm}}}
\begin{tabular}{@{}p{4.7cm}*{12}{c}@{}}
\toprule
 & \multicolumn{2}{c}{Initial} & \multicolumn{7}{c}{Execution} & \multicolumn{3}{c}{Final} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-10}\cmidrule(lr){11-13}
\textbf{Task} & {\tiny W1} & {\tiny W2} & {\tiny W3} & {\tiny W4} & {\tiny W5} & {\tiny W6} & {\tiny W7} & {\tiny W8} & {\tiny W9} & {\tiny W10} & {\tiny W11} & {\tiny W12} \\
\midrule
Project Charter \& planning           & \gb &     &     &     &     &     &     &     &     &     &     &     \\
Environment setup \& patches          & \gb & \gb &     &     &     &     &     &     &     &     &     &     \\
Literature review (state of the art)  &     & \gb & \gb &     &     &     &     &     &     &     &     &     \\
Dataset acquisition                   &     & \gb & \gb &     &     &     &     &     &     &     &     &     \\
Preprocessing \& LMDB build           &     &     & \gb &     &     &     &     &     &     &     &     &     \\
Diagnostic protocol design            &     &     & \gb &     &     &     &     &     &     &     &     &     \\
Guitar v1--v2 (latent failures)       &     &     &     & \gb & \gb &     &     &     &     &     &     &     \\
Guitar v3--v5 (adversarial failures)  &     &     &     &     &     & \gb & \gb &     &     &     &     &     \\
Guitar v6 (partial reconstruction)    &     &     &     &     &     &     &     & \gb & \gb &     &     &     \\
Drums v1--v2 case study               &     &     &     &     &     &     &     & \gb & \gb &     &     &     \\
IRCAM baseline \& calibration studies &     &     &     &     &     &     &     &     & \gb &     &     &     \\
Analysis \& figure generation         &     &     &     &     &     &     &     &     &     & \gb & \gb &     \\
Memoir writing (Chapters 1--6)        &     &     &     &     &     &     &     &     &     & \gb & \gb & \gb \\
\LaTeX\ migration \& defence prep     &     &     &     &     &     &     &     &     &     &     & \gb & \gb \\
Submission (15~June)                  &     &     &     &     &     &     &     &     &     &     &     & \gb \\
\bottomrule
\end{tabular}
\end{table}
