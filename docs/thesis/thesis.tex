...
\usepackage[a4paper,top=2.5cm,bottom=2.5cm,left=2.5cm,right=2.5cm,twoside]{geometry}
...
The launch of RAVE \parencite{caillon2021rave} revolutionized the field by facilitating high-quality synthesis even on easily accessible CPUs to normal users. BRAVE \parencite{caspe2025brave} goes a step further with a causal transmission architecture aimed at creating interactive and responsive music in real-time. However, converting voice to instrument is quite difficult when the source and destination timbres are very different acoustically. For example, plucked string instruments, such as the guitar, have a sharp percussive attack and an exponentially decaying envelope, which is very different from the sustained, formant-rich structure of the human voice. This work investigates if causal transmission systems can achieve voice-to-guitar resynthesis which sounds perceptually convincing to human ears, with a latency of 10\,ms or less so it is usable in live performance. Addressing this cross-modal mismatch is crucial to the enhancement of music interfaces that enable human vocal intuition to be the foundation for machine synthesis.
...
\item \textbf{Qualitative testing:} The author carried out listening tests to evaluate phonetic plausibility and flexibility for live vocal control.
\end{enumerate}

\section{Contributions}\label{sec:contributions}

Deployment and assessment have produced the following technological and methodological contributions:

\begin{itemize}
\item \textbf{Voice-to-drums diagnostic study.} A documented Pure Data setup demonstrating limited transient response of instrument-trained BRAVE models to vocal input. The numerical diagnostic indicates that drums\_v1's decoder responsiveness fell below the 0.01 health threshold (forward difference $=0.0045$; see \S\ref{sec:drums}), contributing evidence that vocal input remains out-of-distribution for both guitar and drums BRAVE models trained on small corpora.
\item \textbf{Guitar autoencoder confirmation.} A working autoencoder, achieved using the official RAVE v2 architecture within a causal framework, for in-distribution input. It serves as a reference point for cross-modal evaluation.
\item \textbf{Failure taxonomy.} A systematic account of five distinct training failure modes observed across the iteration sequence --- posterior collapse (v1), latent underutilisation (v2), discriminator dominance / generator mode collapse (v3, v4, drums\_v2), discriminator collapse (v5), and out-of-distribution decoder behaviour (v6, drums\_v1).
\item \textbf{Dataset heterogeneity exploration.} A numerical investigation revealing noise-floor variability across public guitar datasets that appears likely to be encoded in the latent space as a salient feature (\S\ref{sec:noise_floor}).
\item \textbf{Structural stabilisation.} Empirical evidence that the official RAVE v2 architectural features --- including the 4:1 generator-to-discriminator update ratio and multi-period discriminators \parencite{kong2020hifigan} --- appeared necessary to prevent adversarial mode collapse in the guitar case, although the drums\_v2 iteration suggests they are not sufficient under all training conditions (see \S\ref{sec:drums}).
\item \textbf{External-baseline validation.} A side-by-side diagnostic comparison with the official ACIDS-IRCAM percussion model (\S\ref{sec:ircam_baseline}) indicating that the OOD limitation observed in our models is likely a training-recipe rather than an architectural failure.
\end{itemize}
...
\small
\begin{tabularx}{\textwidth}{lcccXXX}
\toprule
\textbf{Model} & \textbf{Real-time} & \textbf{Causal} & \textbf{Latency} & \textbf{Pitch conditioning} & \textbf{Latent representation} & \textbf{Main limitation for live voice-driven use} \\
\midrule
WaveNet (2016) & No & Yes (autoreg.) & Minutes per second of audio & Optional (linguistic) & None (direct waveform) & Sample-by-sample generation prohibits live use \\
NSynth (2017) & No & Yes (autoreg.) & Minutes per second of audio & None & 16-D temporal embedding & Restricted to isolated notes \\
DDSP (2020) & Yes & No & Real-time capable; bounded by pitch tracker & Explicit F0 + loudness & Explicit DSP control parameters & External pitch tracker on critical path \\
...
Diffusion family (2023--2024) & No & No & Seconds to minutes per generated second & Text / class / melody & Latent diffusion & Inference cost prohibits streaming use \\
\bottomrule
\end{tabularx}
\end{table}
...
\item \textbf{Training on multiple sources under very different recording conditions.} The consequence of mixing acoustic and electric guitar recordings, with the 35.8\,dB noise-floor spread reported in \S\ref{sec:noise_floor}, for BRAVE's latent space is the topic of this work and has not been addressed by prior studies.
\end{itemize}
...
\begin{figure}[H]
\centering
\framebox[\textwidth][c]{\rule{0pt}{2cm}[Block diagram: Mic $\rightarrow$ ADC $\rightarrow$ Pure Data (nn$\sim$ + BRAVE) $\rightarrow$ Gain $\rightarrow$ DAC $\rightarrow$ Speakers]}
\caption{Block diagram of the real-time resynthesis pipeline. Latency contributions per stage are listed in Table~\ref{tab:latency_budget}.}
...
Together, these protocols allow each training iteration in Chapter~\ref{chap:results} to be evaluated against consistent quantitative criteria, while remaining transparent about what those criteria do and do not measure. The implementation details that operationalise this methodology --- software environment, compatibility patches, training scripts, and Pure Data deployment patches --- are described in the following chapter.
...
drums\_v2 & \texttt{drums\_v2\_overrides.gin} & Apply the v6 template to the Groove corpus & Extend the architectural baseline to the drums case study (training initiated; terminated before completion --- discussed in \S\ref{sec:drums}) \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tabularx}{\textwidth}{llXX}
\toprule
\textbf{Iteration} & \textbf{Configuration} & \textbf{Failure mode} & \textbf{Diagnostic signature} \\
\midrule
v1 & Baseline \texttt{c128\_r10.gin} & Posterior collapse & KL falls $0.5 \rightarrow 0.25$; forward diff 0.006 \\
...
v6 & Official RAVE v2 base & Working in-distribution autoencoder & STFT correlation calibrated in Table~\ref{tab:stft_calibration}; GAN gap reversal \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\textbf{Checkpoint} & \textbf{Diagnostic state} & \textbf{Output RMS} & \textbf{STFT corr (raw)} & \textbf{STFT corr (time-aligned)} \\
\midrule
...
guitar\_v6 Phase 1 (epoch 1499) & Reconstruction-only & 0.041 & 0.19 & 0.21 \\
\textbf{guitar\_v6 best (epoch 1935)} & \textbf{Working autoencoder (canonical)} & \textbf{0.024} & \textbf{0.16} & \textbf{0.17} \\
guitar\_v6 final (epoch 2511) & Working, slightly degraded & 0.024 & 0.14 & 0.17 \\
\bottomrule
\end{tabularx}
\end{table}
...
Chapter~\ref{chap:methodology}, \S\ref{sec:datasets_preprocessing} introduced the heterogeneity concern that arises from combining recordings captured through different paths into a single training corpus. To characterise that heterogeneity quantitatively, the noise-floor methodology defined in \S\ref{sec:datasets_preprocessing} was applied to every file of the consolidated 16-hour guitar corpus. For each file, the Root Mean Square energy was computed in non-overlapping 100\,ms windows; the 5th percentile of these RMS values was taken as the noise-floor estimate and converted to dBFS. Per-source aggregates are reported in Table~\ref{tab:noise_floor} and visualised in Figure~\ref{fig:noise_floor}.

\begin{table}[H]
\centering
\caption{Per-source noise-floor distribution across the consolidated guitar training corpus. The 5th-percentile RMS in 100\,ms windows estimates the noise floor of each recording.}
...
\caption{Per-source noise-floor distribution across the consolidated guitar corpus (boxplots; $n=200, 52, 128, 128, 123, 128$ files respectively). Median noise floors range from $-82.4$\,dBFS (IDMT Career SG DI) to $-46.8$\,dBFS (GuitarSet mic-captured acoustic), a 35.8\,dB spread that exceeds typical within-performance dynamic variation.}
\label{fig:noise_floor}
\end{figure}

The spread between the cleanest source (IDMT Career SG direct input, median $-82.4$\,dBFS) and the loudest source (GuitarSet mic-captured acoustic, median $-46.5$\,dBFS) is 35.8\,dB. To contextualise that figure, the typical dynamic variation within a single guitar performance is approximately 20--30\,dB between \textit{pianissimo} and \textit{fortissimo} passages, so the inter-source noise-floor spread exceeds the within-performance dynamic range by an order of magnitude.
...
\paragraph{Project identification.} Title: \emph{BRAVE-Based Real-Time Voice-to-Instrument Resynthesis}. Programme: \thesisDegree, \thesisInstitution, \thesisFaculty. Author: \thesisAuthor. Director: \thesisAdvisor. Period: March 2026 -- June 2026. ECTS: 20 (approximately 500 hours of student work).
...
\begin{table}[H]
\centering
\caption{Three-phase schedule following the UPF TFG guide \parencite{hernandezleo2012tfg}.}
\label{tab:phases}
\begin{tabular}{lccc}
\toprule
\textbf{Phase} & \textbf{Duration} & \textbf{Effort (\% ECTS)} & \textbf{Dates (2026)} \\
\midrule
Initial (planning + setup) & 3 weeks & $\sim$12\,\% & 24 March -- 13 April \\
Execution (iterative training + analysis) & 6 weeks & $\sim$58\,\% & 14 April -- 24 May \\
Final (writing + defence preparation) & $\sim$3 weeks & $\sim$30\,\% & 25 May -- 12 June \\
\bottomrule
\end{tabular}
\end{table}
