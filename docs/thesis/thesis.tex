...
These limitations are not relegated to the conclusions: they constrain how the results in Chapter~\ref{chap:results} should be read, and the contribution claimed by this work (Principal Contribution~2, \S\ref{sec:contributions}) consists in part of stating and quantifying the limit of the STFT-correlation criterion rather than concealing it. The remainder of this section describes the protocols themselves.
...
Figure~\ref{fig:tensorboard} shows these five scalars for the canonical \texttt{guitar\_v6} run, illustrating the healthy pattern against which collapsed iterations are diagnosed in Chapter~\ref{chap:results}: a low, stable KL term, a multi-band spectral distance that decreases through Phase~1 and recovers after the Phase~2 transient, and discriminator logits that remain close rather than diverging.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../../figures/figure_tensorboard_scalars.png}
...
\paragraph{Rationale for the spectral-correlation metric, and why it was later calibrated.} The STFT magnitude correlation was chosen initially for three practical reasons: it is phase-invariant (so it is not penalised by the inaudible phase differences that the spectral training loss also ignores), it is cheap to compute over many candidate checkpoints, and it yields a single bounded scalar that is convenient for ranking. These properties made it attractive as a first-pass automated screen during the iteration sequence, when many checkpoints had to be compared quickly. The need for calibration arose empirically: when the metric was eventually applied across the \emph{full} iteration sequence rather than to a handful of hand-picked checkpoints, it failed to separate working from collapsed models, because the near-silent output of a collapsed decoder still shares the broad low-frequency-dominated spectral envelope of natural audio and therefore correlates trivially with the input. This motivated the cross-checkpoint calibration of \S\ref{sec:stft_calibration}, after which output amplitude and informal listening were retained as the operational evaluators. The episode is reported in full because the failure of an initially reasonable metric is itself one of the principal contributions of this work (\S\ref{sec:contributions}).
...
\paragraph{Dataset heterogeneity.} The guitar corpus mixes microphone-captured acoustic, pickup-captured acoustic, and direct-input electric recordings. The noise-floor analysis of \S\ref{sec:noise_floor} quantifies a 35.9\,dB spread \emph{between} the median estimated noise floors of different source conditions --- that is, a corpus-level inconsistency across recording paths, not a within-signal peak-to-noise-floor (signal-to-noise) ratio of any single file. The work does not control for this heterogeneity by restricting training to a single capture path, and the resulting latent representation is likely to encode recording method as a salient feature, although this was not verified through a controlled latent-clustering experiment.
...
v6 & Official RAVE v2 base & Partial in-distribution reconstruction & STFT corr.\ calibrated in Table~\ref{tab:stft_calibration}; GAN-gap reversal \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../../figures/figure_5_3_spectrogram_in_dist.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../../figures/figure_5_4_drums_v2_gan_widening.png}
