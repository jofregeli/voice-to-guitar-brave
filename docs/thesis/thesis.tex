...
\item \textbf{Original contribution 5 --- Dataset-heterogeneity quantification.} A noise-floor analysis (\S\ref{sec:noise_floor}, Table~\ref{tab:noise_floor}) measuring a 35.8\,dB spread across six recording paths within the consolidated guitar corpus, with documented implications for what features the encoder is likely to internalise as salient.
...
\paragraph{Dataset heterogeneity.} The guitar corpus mixes microphone-captured acoustic, pickup-captured acoustic, and direct-input electric recordings. The noise-floor analysis of \S\ref{sec:noise_floor} quantifies a 35.8\,dB spread between source conditions. The work does not control for this heterogeneity by restricting training to a single capture path, and the resulting latent representation almost certainly encodes recording method as a salient feature.
