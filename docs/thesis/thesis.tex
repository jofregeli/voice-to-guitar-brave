...
\begin{table}[H]
\centering
\caption{Composition of the guitar training corpus after format filtering (mono, 16-bit PCM, 44.1\,kHz). The IDMT-SMT-Guitar dataset~2 was set aside during this research phase to keep the pipeline restricted to natively 16-bit source material; although converting its 24-bit files would have been straightforward, this was not pursued in order to avoid introducing a re-quantisation step into the corpus. After preprocessing, the resulting LMDB reports \texttt{n\_seconds = 57665} ($\sim$16\,h); the higher figure reflects overlapping segmentation rather than additional source audio.}
